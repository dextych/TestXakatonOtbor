...
//КОНФИГУРАЦИЯ КОМНАТЫ С ПОВТОРОМ (из конфига)
    ...
    test('Шаг 4: Ожидание активации комнаты', async () => {
        ...
        if (waitTime > 0 && waitTime < 120000) {
            ...
            if (roomActivated) {
                ...
                console.log(`\nКомната не активировалась за отведенное время`);
            }
            
        } else {
            console.log(`\nВремя ожидания слишком большое (${Math.round(waitTime / 1000)} сек), пропускаем`);
        }
    }, 180000);
    ...
    afterAll(async () => {
        ...
        console.log('\n=== СТАТИСТИКА ТЕСТА ===');
        console.log(`Комната создана: ${createdRoom ? 'Да' : 'Нет'}`);
